...
REM ============================================================================
REM  Build OpenSSL 3.x/4.x for Win32 using Microsoft Visual C (MSVC)
REM
REM  Prerequisites:
...
REM
REM  Usage:
REM    build_openssl3_msvc_win32.bat [no-asm]
REM
REM  Output:
REM    .\%OBJ_DIR%\win32\vc\*_win32.obj  -  object files for static linking with Delphi
REM ============================================================================
...
call "%~dp0build_config.bat" msvc_win32
...
echo [3/7] Configuring OpenSSL for VC-WIN32...
...
set "CONFIGURE_OPTS=VC-WIN32 no-makedepend no-shared no-tests no-module"
...
REM                    public API wrappers. See helpers\openssl_fix_case_collisions.h.
...
REM Uses inclusion filter instead of exclusion to avoid collecting apps/template/test objects.
...
echo [7/7] Generating %OBJ_DIR%_L_win32_vc.inc...

set "INC_FILE=%~dp0%OBJ_DIR%_L_win32_vc.inc"
set "INC_PATH=%OBJ_DIR%\win32\vc"
set "INC_COUNT=0"
...
echo   Generated !INC_COUNT! {$L} directives in %OBJ_DIR%_L_win32_vc.inc

REM === Verify: check for uncovered case-insensitive collisions ===
echo.
echo [POST] Checking for uncovered symbol collisions...
python "%~dp0helpers\gen_case_collision_fixes.py" --verify --compiler msvc
if errorlevel 1 (
    echo.
